		...
		while(!requiredFieldsComplete){
			...
			try {
				if (stars == null || stars.length < 1) {
					...
					if (!stars[0].first_name.equals("") && stars[0].last_name.equals("")) {
						stars[0].last_name = stars[0].first_name;
						stars[0].first_name = "";
					}
				}
				
				requiredFieldsComplete = !title.equals("") && !(year < 1000 || year > 9999) && !director.equals("") && !genres[0].name.equals("") && !stars[0].last_name.equals("");
			} catch (Exception e) {
